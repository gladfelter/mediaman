...
class PhotoManFunctionalTests(unittest.TestCase):
    ...
    def testNewDatabase(self):
        ...
        try:
            ...
            files = [
                ...
                os.path.join(tmpdir,
                             'dest/media/photos/2012/07_July/'
                             'gnexus 160.jpg'),
                os.path.join(tmpdir, 'dest/media/photos/2002/'
                             '10_October/105-0555_IMG.JPG'),
                os.path.join(tmpdir,
                             'dest/media/photos/2003/03_March/'
                             '594-9436_IMG.JPG'),
                os.path.join(tmpdir,
                             'dest/media/photos/2006/03_March/'
                             'IMG_1427.JPG'),
                os.path.join(tmpdir,
                             'dest/media/photos/2006/06_June/'
                             'DSC09012.JPG'),
    ...
    def test_scan_missing(self):
        ...
        try:
            ...
            rep = media_common.Repository()
            rep.open(os.path.join(mediadir))
            self.assertEqual(rows - 1, self._get_row_count(rep))
            cur = rep.con.cursor()
            ...
            self.assertEqual(None, rows.fetchone())
            rep.close()
        finally:
            shutil.rmtree(tmpdir)
